...
[代码随想录题解](https://programmercarl.com/0226.%E7%BF%BB%E8%BD%AC%E4%BA%8C%E5%8F%89%E6%A0%91.html#%E6%80%9D%E8%B7%AF)
...
<img src="./assets/image-20240126020140630.png" alt="image-20240126020140630" style="zoom:50%;" />
...
![image-20240126020455249](./assets/image-20240126020455249.png)
...
[代码随想录题解](https://programmercarl.com/0101.%E5%AF%B9%E7%A7%B0%E4%BA%8C%E5%8F%89%E6%A0%91.html#%E7%AE%97%E6%B3%95%E5%85%AC%E5%BC%80%E8%AF%BE)
...
<img src="./assets/image-20240126200612437.png" alt="image-20240126200612437" style="zoom: 50%;" />
...
[代码随想录题解](https://programmercarl.com/0104.%E4%BA%8C%E5%8F%89%E6%A0%91%E7%9A%84%E6%9C%80%E5%A4%A7%E6%B7%B1%E5%BA%A6.html)
...
还是一个关于二叉树遍历的题目，本题是针对前序和后序的一个综合考察

<img src="./assets/image-20240128211233201.png" alt="image-20240128211233201" style="zoom:80%;" />

定义一个 `tempDepth` 变量记录深度初始值设为 `0`

以节点 `20` 为例，如果要记录该节点的深度，就应该在前序位置（刚进入节点的位置）执行`tempDepth++` 

<img src="./assets/image-20240128211632674.png" alt="image-20240128211632674" style="zoom:80%;" />

上图是遍历步骤的执行顺序，`tempDepth` 的值在红色部分，也就是前序遍历的时候进行自增，而在蓝色部分，即返回上一级节点的时候进行自减，这时候的 `tempDepth` 就是该节点所在的层数，也就是要求的深度。

`tempDepth` 达到最大值的时候一定是叶子节点的位置，即 `node == null` 的时候，每当抵达叶子节点的时候与之前求得的最大值作比较即可确定最大值。
...
[代码随想录题解](https://programmercarl.com/0111.%E4%BA%8C%E5%8F%89%E6%A0%91%E7%9A%84%E6%9C%80%E5%B0%8F%E6%B7%B1%E5%BA%A6.html)
...
与最大深度相比，最小深度要考虑的内容要多一些，不能再采用上面的 `node == null` 的时候来获取节点，因为会存在如下的情况，对应着示例 2：

<img src="./assets/image-20240128221125680.png" alt="image-20240128221125680" style="zoom:80%;" />

即将 `left` 误判为根节点，如果还按照最大的写法来写的话，会发现输出的是 `1`，因为在 `2` 位置因为其 `left` 为 `null` ，执行了 `min = Math.min(min, tempDepth);` 的语句，但上题求得的是最大值，无需考虑这种情况。

所以我们要改变计数的位置，即当 `node.right == null && node.left == null` 的时候，才是真正的根节点；很容易写出代码
...
当判断节点的左节点与右节点均为空的时候再去做比较赋值操作，就可以实现了         吗？。

但我把完整代码写在笔记部分肯定是有些问题。
...
很好回答，是前序位置，在前序位置直接做了返回操作，如果后序位置没有操作的时候显然没问题，但是本题中，在后序位置做了 `tempDepth--` 的操作，直接返回出现了问题，这也就是提交后出错的原因。
...
如果对前序、后序中序的描述有问题，可以查看我往期的题解部分。

最后来考虑剪枝问题，即当 `tempDepth` 已经大于等于 `min` 的时候就不需要继续进行递归了。
...
### 03. 完全二叉树的节点个数（No. 222）

[题目链接](https://leetcode.cn/problems/count-complete-tree-nodes/description/)

[代码随想录题解](https://programmercarl.com/0222.%E5%AE%8C%E5%85%A8%E4%BA%8C%E5%8F%89%E6%A0%91%E7%9A%84%E8%8A%82%E7%82%B9%E4%B8%AA%E6%95%B0.html)
...
先来实现简单的二叉树遍历，即将其当作普通的二叉树去处理，为了深入理解，这里实现前序和后序两种方式。

前序很好理解，就是在进入节点的时候执行计数，最后将整个结果输出。
...
所谓后序遍历，就是收集左右节点的值作为自己的值，而不断从下往上计算
...
class Solution {
    ...
    public int reverse(TreeNode node) {
        ...
        int rightNum = reverse(node.left);
        int leftNum = reverse(node.right);
        int num = rightNum + leftNum + 1;
        return num;
    }
}
```
...
再来尝试利用完全二叉树的性质，完全二叉树的定义在前面博客中提到过，这里简单介绍一下，完全二叉树是除了倒数第二层以外，其他的位置都有两个子节点，且最后一层有节点的位置是连续的，利用这个性质可以少遍历一些节点。

![image-20240128232900842](./assets/image-20240128232900842.png)

已知满二叉树的节点数为 2^n^ - 1，所以可以只遍历两个外层的节点，如果发现两个外层的层数（深度）相同的话，就说明是一个满二叉树，这个结论仅在满足二叉树是完全二叉树的时候成立，如上图，如果两个外侧的节点数相同，则因为完全二叉树的性质，红圈部分一定有节点。



所以可以不断检查节点的子树是否是完全二叉树，如果是，直接通过公式 2^n^ - 1 来计算，即在遍历之前加入一段判断逻辑
...
class Solution {
    ...
    public int reverse(TreeNode node) {
        ...
        // 后序遍历部分
        int leftTreeNum = reverse(node.left);
        int rightTreeNum = reverse(node.right);
        int num = leftTreeNum + rightTreeNum + 1;
        return num;
    }
}
```
...
[代码随想录题解](https://programmercarl.com/0110.%E5%B9%B3%E8%A1%A1%E4%BA%8C%E5%8F%89%E6%A0%91.html)
...
<img src="./assets/image-20240129132443871.png" alt="image-20240129132443871" style="zoom:50%;" />

接下来将其改造为展示高度的图像

<img src="./assets/image-20240129132526761.png" alt="image-20240129132526761" style="zoom:50%;" />
...
class Solution {
    ...
    public int reverse(TreeNode node) {
        ...
        int rightNum = reverse(node.right);
        int leftNum = reverse(node.left);
        // 判断是否平衡
        if (rightNum - leftNum > 1 || leftNum - rightNum > 1) {
            flg = false;
        }
...
[代码随想录题解](https://programmercarl.com/0257.%E4%BA%8C%E5%8F%89%E6%A0%91%E7%9A%84%E6%89%80%E6%9C%89%E8%B7%AF%E5%BE%84.html#%E7%AE%97%E6%B3%95%E5%85%AC%E5%BC%80%E8%AF%BE)
...
[代码随想录刷题笔记 DAY16 | 二叉树的最大深度 No.104 | 二叉树的最小深度 No.111 | 完全二叉树的节点个数 No.222](https://blog.csdn.net/weixin_74895237/article/details/135902957)
...
- 确认递归的出口，本题有两个出口，第一个是 `node == null` 也就是遍历到空节点的时候，但空节点并非一定是叶子节点，比如下图
  <img src="./assets/image-20240129153626406.png" alt="image-20240129153626406" style="zoom:50%;" />
  如果仅仅通过 `node == null` 来判断，这些节点的上一个节点都会被判定为叶子节点，也就是 `1` `2` `3` 节点均会判定为叶子节点，所以还需要限制 `left == null` 和 `right == null` 这才能准确的定位到 `3` 节点。
...
```java
class Solution {
    List<String> res = new ArrayList<String>(); // 返回的结果
    List<String> path = new ArrayList<String>(); // 收集路径信息
    ...
    public void reverse(TreeNode node) {
        ...
        // 判断是否是叶子节点
        if (node.right == null && node.left == null) {
            String resString = getResString(path);
            res.add(resString);
    ...
    /*
    	将 path 转为需要的结果的方法
    */
    public String getResString(List<String> path) {
        String resString = "";
        for (int i = 0; i < path.size() - 1; i++) {
            resString += path.get(i) + "->";
        }
        resString += path.get(path.size() - 1);
        return resString;
    }
}
```
...
[代码随想录题解](https://programmercarl.com/0404.%E5%B7%A6%E5%8F%B6%E5%AD%90%E4%B9%8B%E5%92%8C.html#%E7%AE%97%E6%B3%95%E5%85%AC%E5%BC%80%E8%AF%BE)
...
<img src="./assets/image-20240129155719305.png" alt="image-20240129155719305" style="zoom:50%;" />

如果问这张图的左叶子之和很有可能会得到两个结果，`9 + 3 + 15 = 27 ` 和 `3 + 15 = 18`，凭字面理解，很明显是第二个对了，因为要找的是 **左叶子节点** 而非 **左分支节点**，所以和上题一样，要加判断是否为叶子节点的代码，我认为这题很适合作为递归的一个例题，因为本题的递归三步法比较复杂。
...
- 递归中执行的内容：（前序位置：检查是否为空节点、拿取节点**左叶子**的值（`if not null`））、（遍历左节点、遍历右节点）、（后续位置：返回当前节点以下所有 **左叶子** 的值）
...
class Solution {
    ...
    public int reverse(TreeNode node) {
        ...
        // 判断左叶子
        if (node.left != null && node.left.right == null && node.left.left == null) {
            // 当前节点的左叶子的值（如果存在的话），注意是左叶子
            temp = node.left.val;
        }
        int leftNum = reverse(node.left);
        int rightNum = reverse(node.right);
        return leftNum + rightNum + temp;
    }
}
```
...
[代码随想录题解](https://programmercarl.com/0513.%E6%89%BE%E6%A0%91%E5%B7%A6%E4%B8%8B%E8%A7%92%E7%9A%84%E5%80%BC.html#%E7%AE%97%E6%B3%95%E5%85%AC%E5%BC%80%E8%AF%BE)
...
<img src="./assets/image-20240130144042211.png" alt="image-20240130144042211" style="zoom:50%;" />
...
[代码随想录题解](https://programmercarl.com/0112.%E8%B7%AF%E5%BE%84%E6%80%BB%E5%92%8C.html)
...
[代码随想录题解](https://programmercarl.com/0106.%E4%BB%8E%E4%B8%AD%E5%BA%8F%E4%B8%8E%E5%90%8E%E5%BA%8F%E9%81%8D%E5%8E%86%E5%BA%8F%E5%88%97%E6%9E%84%E9%80%A0%E4%BA%8C%E5%8F%89%E6%A0%91.html#%E5%85%B6%E4%BB%96%E8%AF%AD%E8%A8%80%E7%89%88%E6%9C%AC)
...
<img src="./assets/image-20240130192339096.png" alt="image-20240130192339096" style="zoom:50%;" />
...
![image-20240130202726874](./assets/image-20240130202726874.png)
